...
function buildRow(lead, dateObj, dateKey, idx) {
  ...
  // Attach data attributes for row identification
...
// Helper: Find lead by row
function findLeadByRow(tr) {
  const dateKey = tr.dataset.date;
  const idx = parseInt(tr.dataset.idx, 10);
  const leads = appState.data[appState.currentMonth] ?? [];
  // Find all leads for this date
  const leadsForDate = leads.filter(l => {
    let d = new Date(l.date);
    d.setHours(0,0,0,0);
    return d.toISOString() === dateKey;
  });
  const lead = leadsForDate[idx];
  return [leads, lead, dateKey, idx, leadsForDate];
}

// Input and select change
document.getElementById('leadsTableBody').addEventListener('input', function (e) {
  const inp = e.target;
  if (!inp.matches('input[data-field]')) return;
  const tr = inp.closest('tr');
  if (!tr) return;
  const [leads, lead, dateKey, idx, leadsForDate] = findLeadByRow(tr);
  if (!lead) return;
  lead[inp.dataset.field] = inp.value;
  saveData();
});
document.getElementById('leadsTableBody').addEventListener('change', function (e) {
  ...
  const [leads, lead, dateKey, idx, leadsForDate] = findLeadByRow(tr);
  if (!lead) return;
  lead[sel.dataset.field] = sel.value;
  saveData();
  render(); // To show new status/emoji
});

// Counter buttons
document.getElementById('leadsTableBody').addEventListener('click', function (e) {
  const btn = e.target.closest('button.counter-btn');
  if (!btn) return;
  ...
  saveData();
  render(); // re-render table after counter changed
});

// Contact made buttons
document.getElementById('leadsTableBody').addEventListener('click', function (e) {
  ...
  const key = "contactMade" + type.charAt(0).toUpperCase() + type.slice(1);
  lead[key] = !lead[key];
  if (lead[key] && !lead.contactMadeOn) lead.contactMadeOn = new Date().toISOString();
  saveData();
  render(); // re-render table to update status/emoji
});
...
document.getElementById('leadsTableBody').addEventListener('click', function (e) {
  ...
  lead.hot = !lead.hot;
  saveData();
  render(); // re-render table to update hot emoji/status
});

// Delete row button
document.getElementById('leadsTableBody').addEventListener('click', function (e) {
  const btn = e.target.closest('button.delete-row-btn');
  if (!btn) return;
  const tr = btn.closest('tr');
  if (!tr) return;
  const [leads, lead, dateKey, idx, leadsForDate] = findLeadByRow(tr);
  if (!lead) return;
  const allLeads = appState.data[appState.currentMonth] ?? [];
  const leadIdx = allLeads.indexOf(lead);
  if (leadIdx > -1) {
    allLeads.splice(leadIdx, 1);
    appState.data[appState.currentMonth] = allLeads;
    saveData();
    render();
  }
});
